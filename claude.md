...
`deno run -A pipeline.ts [stage]` — stages: `list`, `embed`, `tag`, `dedupe`, `difficulty`, `prereq`, `postproc`, `finalize` (default: all).

Monolithic Deno script (~5,300 lines) that parses all datasets into a unified schema, infers prerequisites from student interaction data, merges duplicate skills by label, and writes output files. Stage outputs cached in `build/`.
...
All generated by `pipeline.ts` into the project root. Gitignored.

### skills.tsv / skills.tsv.gz — Unified Skill Table

~99k rows. Columns: `id, title, display_title, description, difficulty, prereqs, occupations, topics, grade_start, grade_end`
