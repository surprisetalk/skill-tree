...
~382k rows. Columns: `skill_id, prereq_id, source, type`

- `type` — either `"prerequisite"` (true learning dependency) or `"broader"` (taxonomic containment from LCSH)
- `source` — origin dataset: `khan`, `alcpl`, `metacademy`, `esco`, `opensalt`, `mooccubex`, `junyi_hierarchy`, `junyi_logs`, `assistments_logs`, `llm`, `lcsh_broader`
