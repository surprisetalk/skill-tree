...
~449k rows. Columns: `skill_id, prereq_id, source, type`

- `type` — either `"prerequisite"` (true learning dependency) or `"broader"` (taxonomic containment from LCSH)
- `source` — origin dataset: `khan`, `alcpl`, `metacademy`, `esco`, `esco_optional`, `opensalt`, `mooccubex`, `junyi_hierarchy`, `junyi_logs`, `assistments_logs`, `csp_grade`, `llm`, `lcsh_broader`
